<template>
  <div class="hello">
    <el-row type="flex" class="row-bg" justify="center" align="middle">
      <el-col :span="4">
        房态：<el-select v-model="searchParam.status" clearable placeholder="请选择" @change="roomStatusChange">
          <el-option
      ...
      <el-col :span="20">
        房间号：<el-input v-model="searchParam.roomNum" @input="roomStatusChange" placeholder="房间号" style="width: 217px"></el-input>
      </el-col>
    </el-row>
    ...
    <el-dialog title="空房" :visible.sync="showVacant" width="30%">
              ...
              <el-tabs value="first">
                <el-tab-pane label="租房信息" name="first">
                  ...
                  <el-form-item label="证件类型" prop="cardType">
                    <el-select v-model="roomForm.cardType" @change="cardTypeChange" clearable placeholder="证件类型">
                      <el-option label="身份证" value="1"></el-option>
                      <el-option label="其他" value="2"></el-option>
                    </el-select>
                  </el-form-item>
                  <el-form-item label="证件号" prop="cardNo">
                    <el-input v-model="roomForm.cardNo" placeholder="证件号" maxlength="18" style="width: 217px"></el-input>
                  </el-form-item>
                  ...
                  <el-form-item label="手机号" prop="mobilePhone">
                    <el-input v-model="roomForm.mobilePhone" maxlength="11" placeholder="手机号" style="width: 217px"></el-input>
                  </el-form-item>
                ...
                <el-tab-pane label="合住人与紧急联系人" name="first">
                  <div v-for="item in roomForm.cohabitant" :key="item.id">
                    ...
                    <el-form-item label="证件类型">
                      <el-select v-model="item.cardType" @change="sharingCardTypeChange(item)" clearable placeholder="证件类型">
                        <el-option label="身份证" value="1"></el-option>
                        <el-option label="其他" value="2"></el-option>
                      </el-select>
                    </el-form-item>
                  ...
                  <el-form-item label="联系人手机号" prop="urgentPhone">
                    <el-input v-model="roomForm.urgentPhone" placeholder="联系人手机" maxlength="11" style="width: 217px"></el-input>
                  </el-form-item>
                </el-tab-pane>
              </el-tabs>
            </el-col><el-col :span="2">1</el-col>
              ...
              <el-tabs value="first">
                <el-tab-pane label="房租费用" name="first">
                  <!--<el-form-item label="预订号码">
                    <el-input v-model="reserveMobile" placeholder="预订号码" maxlength="11" style="width: 217px"></el-input>
                    <el-button type="primary" icon="el-icon-search" size="mini" @click="queryReserve" style="margin-left: 5px">查询</el-button>
                  </el-form-item>-->
                  ...
                  <el-form-item label="成交房价" prop="realPrice">
                    <el-input v-model="roomForm.realPrice" placeholder="成交房价" maxlength="4" style="width: 217px" @blur="realPriceChange()"></el-input>
                  </el-form-item>
                  ...
                  <el-form-item label="收款方式" prop="payType">
                    ...
                    <el-select v-model="roomForm.payType" clearable placeholder="支付方式">
                      <el-option v-for="item in payTypeMap" :key="item.id" :label="item.key" :value="item.value"></el-option>
                    </el-select>
                  </el-form-item>
      ...
      <el-dialog title="房间预订" :visible.sync="showReserve" :show-close="false" width="30%" append-to-body>
        <el-form ref="reserveForm" :model="reserveForm" :rules="rules" label-width="110px">
          ...
          <el-form-item label="预订手机号" prop="reserveMobile">
            <el-input v-model="reserveForm.reserveMobile"  placeholder="预订手机号" maxlength="11" style="width: 217px"></el-input>
          </el-form-item>
          <el-form-item label="订金金额" prop="reservePrice">
            <el-input v-model="reserveForm.reservePrice"  placeholder="订金金额" maxlength="4" style="width: 217px"></el-input>
          </el-form-item>
      ...
      <el-dialog title="员工入住" :visible.sync="showStaff" width="50%" :show-close="false" append-to-body>
        <div>房间号：{{this.roomForm.roomNum}}</div><br/>
          ...
          <el-collapse-item>
            ...
            <el-table :data="enterResult.rents" border style="width: 100%">
              ...
              <el-table-column label="状态">
                <template slot-scope="scope">
                  <el-button :type="scope.row.status=='0'?'danger':scope.row.status=='1'?'success':'warning'" plain disabled>{{rentStatusMap[scope.row.status]}}</el-button>
                </template>
              </el-table-column>
            </el-table>
          </el-collapse-item>
          ...
          <el-tab-pane label="设备/装修损坏信息" name="first" >
            <el-row v-for="(item,index) in refundForm.damageArr" :key="index">
              ...
              <el-col :span="6" v-if="index+1 === refundForm.damageArr.length">
                <el-button type="primary" icon="el-icon-plus" size="mini" circle @click="addDamage" style="margin-left: 5px"></el-button>
                <el-button type="primary" icon="el-icon-minus" size="mini" circle @click="removeDamage(index)" style="margin-left: 5px"></el-button>
              </el-col>
            </el-row>
          </el-tab-pane>
        </el-tabs>
...
export default {
  ...
  data: function() {
    return {
      ...
      rules: {
        ...
        sharingCardNo:[
          { pattern: '^[1-9]\\d{5}(18|19|20|(3\\d))\\d{2}((0[1-9])|(1[0-2]))(([0-2][1-9])|10|20|30|31)\\d{3}[0-9Xx]$', message: '身份证号格式不正确', trigger: 'blur',flag:'cardNoValid' }
        ],
        permanentAddress: [
  ...
  methods:{
    ...
    queryStaffRoom(roomId,roomNum){
      this.$http.post(pixUrl+'/integration/queryStaffRoom',{'roomId':roomId,'roomNum':roomNum}).then(function(res){
        const result = res.body;
        if(result.ok){
          ...
          this.$http.post(pixUrl+'/reserve/saveReserve',this.reserveForm).then(function(res){
            ...
            if(result.ok){
              this.$notify({
                ...
                duration:1500,
                offset: 100,
                onClose: ()=>{ this.roomStatusChange();this.reserveForm={};this.showReserve=false;this.showVacant=false; }
              });
            }else{
              this.$notify({
                title: '提醒',
          ...
          this.$http.post(pixUrl+'/integration/checkIn',this.roomForm).then(function(res){
            ...
            if(result.ok){
              this.$notify({
                ...
                duration:1500,
                offset: 100,
                onClose: ()=>{ this.roomStatusChange();this.roomForm={};this.showCheckIn=false;this.showVacant=false; }
              });
            }
          },function(){
            console.log('保存信息失败');
    ...
    saveStaffCheckIn(){
      this.$http.post(pixUrl+'/integration/staffCheckIn',{'roomId':this.roomForm.roomId,'roomNum':this.roomForm.roomNum,'staffIds':this.staffCheckIn}).then(function(res){
        const result = res.body;
        if(result.ok){
          this.$notify({
            title: '提醒',
            ...
            duration:1500,
            offset: 100,
            onClose: ()=>{ this.roomStatusChange();this.roomForm={};this.showStaff=false;this.showVacant=false; }
          });
        }
      },function(){
        console.log('by手机号预定信息');
    ...
    cardTypeChange(){
      if(this.roomForm.cardType==='1'){
        this.rules.cardNo.push({ pattern: '^[1-9]\\d{5}(18|19|20|(3\\d))\\d{2}((0[1-9])|(1[0-2]))(([0-2][1-9])|10|20|30|31)\\d{3}[0-9Xx]$', message: '身份证号格式不正确', trigger: 'blur',flag:'cardNoValid' })
      }else{
        let item = ''
    ...
    sharingCardTypeChange(item){
      if(item.cardType==='1'){
        this.rules.sharingCardNo.push({ pattern: '^[1-9]\\d{5}(18|19|20|(3\\d))\\d{2}((0[1-9])|(1[0-2]))(([0-2][1-9])|10|20|30|31)\\d{3}[0-9Xx]$', message: '身份证号格式不正确', trigger: 'blur',flag:'cardNoValid' })
      }else{
